...
/** Fixed UUIDs for reproducible dev/tests (not for production credentials). */
...
async function main() {
  ...
  // Optional: partner can view owner's individual data (demo permission)
  ...
  // Individual one-time expense (Alvaro)
  ...
  // Shared expense with explicit equal split rows (50/50)
  ...
  // Fixed monthly expense via recurrence (rent share — individual)
  ...
  // Installment group example: 1200 in 6x from July 2026 (metadata + first months seeded)
  ...
  for (let i = 0; i < 6; i++) {
    const ref = new Date(2026, 6 + i, 1); // July = month 6 in 0-based
    await prisma.expenseOccurrence.create({
      data: {
  ...
  // Future credit card expense (first charge June 2026)
