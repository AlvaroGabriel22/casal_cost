...
let BankStatementAnalysisService = class BankStatementAnalysisService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async analyze(userId, referenceMonth, contextRules = []) {
        ...
        const { lots, currentlyInvested, totalApplied, totalRedeemed } = this.trackInvestmentLots(entries);
        const investmentLots = lots.map((lot) => this.lotToTracking(lot, entries));
        const savedInAccountTotal = monthlyBreakdown.reduce((sum, m) => sum + Math.max(m.savedInAccount, 0), 0);
        const refBreakdown = monthlyBreakdown.find((m) => m.month === referenceMonth) ?? null;
    ...
    formatForRag(analysis) {
        if (!analysis.hasData)
            return [];
        const docs = [];
        docs.push(`[Análise do extrato bancário] ${analysis.totalMovements} movimentações em ${analysis.monthsCovered.length} mês(es) (${analysis.banks.join(', ')}). Investido atualmente: ${this.brl(analysis.currentlyInvested)}. Total aplicado: ${this.brl(analysis.totalAppliedAllTime)}. Total resgatado: ${this.brl(analysis.totalRedeemedAllTime)}. Guardado em conta (acumulado): ${this.brl(analysis.savedInAccountTotal)}.`);
        for (const month of analysis.monthlyBreakdown) {
            docs.push(`[Extrato ${month.month}] entradas: ${this.brl(month.totalIn)} | saídas: ${this.brl(month.totalOut)} | saldo: ${this.brl(month.net)} | aplicado em investimentos: ${this.brl(month.investedApplied)} | resgatado: ${this.brl(month.investedRedeemed)} | guardado em conta: ${this.brl(month.savedInAccount)} | ${month.transactionCount} lançamentos.`);
        }
        ...
        if (spend) {
            docs.push(`[Gastos extrato ${spend.referenceMonth}] entradas ${this.brl(spend.totalIncome)} | gastos ${this.brl(spend.totalSpent)} | saldo ${this.brl(spend.balance)}${spend.spentMoreThanEarned ? ` | déficit ${this.brl(spend.overspendAmount)}` : ''}. ${spend.deficitReason}`);
            for (const cat of spend.topCategories.slice(0, 5)) {
                docs.push(`[Gasto por categoria ${cat.category}] ${this.brl(cat.total)} (${cat.sharePercent.toFixed(0)}% dos gastos, ${cat.count} lançamentos).`);
            }
            for (const rec of spend.recurringExpenses.slice(0, 12)) {
                docs.push(`[Gasto recorrente] ${rec.label} (${rec.category}): ${rec.occurrences}x, média ${this.brl(rec.averageAmount)}, total ${this.brl(rec.totalInPeriod)}${rec.isCrossMonthRecurring ? ', em vários meses' : ''}.`);
            }
            const taught = spend.expenseDetails.filter((d) => d.userMotive);
            for (const detail of taught) {
                docs.push(`[Gasto explicado pelo usuário] ${detail.date} ${detail.merchant} ${this.brl(detail.amount)} [${detail.category}]: ${detail.userMotive}`);
            }
            ...
            for (const detail of spend.expenseDetails.slice(0, 40)) {
                docs.push(`[Gasto ${detail.category}] ${detail.date} ${detail.merchant} ${this.brl(detail.amount)} — ${detail.detail}${detail.isRecurring ? ' (recorrente)' : ''}`);
            }
        }
        return docs;
    }
    ...
    buildMonthlyBreakdown(entries, months) {
        return months.map((month) => {
            ...
            for (const e of monthEntries) {
                ...
                if (e.type === 'INVESTMENT_APPLY')
                    investedApplied += e.amount;
                if (e.type === 'INVESTMENT_REDEEM')
                    investedRedeemed += e.amount;
                if (e.direction === 'CREDIT' &&
    ...
    trackInvestmentLots(entries) {
            ...
            if (entry.type === 'INVESTMENT_REDEEM') {
                ...
                for (const lot of lots) {
                    ...
                    if (!lot.firstRedemptionDate)
                        lot.firstRedemptionDate = entry.date;
        ...
        const currentlyInvested = this.round(lots.reduce((sum, lot) => sum + lot.remaining, 0));
        return {
            lots,
            currentlyInvested,
    ...
    lotToTracking(lot, allEntries) {
        const redeemed = this.round(lot.originalAmount - lot.remaining);
        const status = lot.remaining <= 0 ? 'REDEEMED' : redeemed > 0 ? 'PARTIAL' : 'ACTIVE';
        let monthsHeld = 0;
        if (lot.firstRedemptionDate) {
    ...
    composeInsights(input) {
        const insights = [];
        let seq = 0;
        const add = (tone, title, message, tip) => {
            insights.push({
                id: `bank-insight-${++seq}`,
                tone,
                title,
        ...
        if (input.refBreakdown) {
            ...
            if (m.investedApplied > 0) {
                add('POSITIVE', `Investimento em ${refLabel}`, `Aplicou ${this.brl(m.investedApplied)} em RDB${m.investedRedeemed > 0 ? ` e resgatou ${this.brl(m.investedRedeemed)}` : ''}.`);
            }
            if (m.investedRedeemed > m.investedApplied && m.investedRedeemed > 0) {
                add('ATTENTION', `Resgates acima de aplicações em ${refLabel}`, `Resgatou ${this.brl(m.investedRedeemed)} e aplicou ${this.brl(m.investedApplied)}.`);
            }
        }
        if (input.currentlyInvested > 0) {
            add('POSITIVE', 'Patrimônio investido no RDB', `Há ${this.brl(input.currentlyInvested)} ainda aplicados, rastreados a partir do extrato.`);
        }
        return insights.slice(0, 3);
    }
    composeRecommendations(input) {
        const tips = [];
        if (!input.refBreakdown) {
        ...
        if (m.net < 0 && !input.spendingAnalysis?.spentMoreThanEarned) {
            tips.push('O fluxo bruto fechou negativo por resgates/aplicações — seu consumo real está na análise de gastos.');
        }
    ...
    composeSpendingAnalysis(input) {
        ...
        const monthIncome = input.entries.filter((e) => e.month === input.referenceMonth &&
            e.direction === 'CREDIT' &&
            e.type !== 'INVESTMENT_REDEEM');
        const totalIncome = this.round(monthIncome.reduce((s, e) => s + e.amount, 0));
        const totalSpent = this.round(monthSpending.reduce((s, e) => s + e.amount, 0));
        ...
        const enrichedSpending = monthSpending.map((e) => {
            ...
            const rule = (0, finance_context_matcher_1.findMatchingRule)(input.contextRules, e.description, merchant);
            const fromDesc = (0, category_guess_1.inferSpendingCategory)(e.description);
            const inferred = fromDesc !== 'Outros' ? fromDesc : (0, category_guess_1.inferSpendingCategory)(merchant);
            ...
            return {
                ...
                matchLabel: (0, finance_context_matcher_1.normalizeMatchLabel)(merchant),
            };
        });
        ...
        const expenseDetails = enrichedSpending
        ...
        const largest = enrichedSpending.reduce((max, e) => (!max || e.amount > max.amount ? e : max), null);
        const largestExpense = largest
            ...
            : null;
        const knownLabels = new Set(input.contextRules.map((r) => r.matchLabel));
        const smallRecurringLeaks = recurringExpenses
            .filter((r) => r.averageAmount <= SMALL_EXPENSE_THRESHOLD &&
            r.occurrences >= 2 &&
            r.annualizedEstimate >= 50 &&
            !knownLabels.has((0, finance_context_matcher_1.normalizeMatchLabel)(r.label)))
        ...
        const actions = this.buildDirectActions({
            ...
            largestExpense,
            smallRecurringLeaks,
            recurringExpenses: recurringExpenses.filter((r) => r.lastSeenMonth === input.referenceMonth),
            expenseDetails,
        });
        return {
    ...
    buildSmartExpenseDetail(category, merchant, motive) {
        const m = motive.trim();
        const name = this.shortMerchantName(merchant);
        const lower = m.toLowerCase();
        const parcelaAmount = lower.match(/parcela[^.]*?(\d{2,5})\s*reais?/) ??
            lower.match(/parcela\s*(?:é|e|de)\s*(\d{2,5})/);
        const monthEnd = lower.match(/(janeiro|fevereiro|março|marco|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)\s*(?:de\s*)?(\d{4})/);
    ...
    motiveGist(motive, maxLen = 55) {
        ...
        if (!s)
            return motive.slice(0, maxLen);
        return s.charAt(0).toUpperCase() + s.slice(1);
    }
    shortMerchantName(merchant) {
    ...
    expenseDetailLabel(category, merchant) {
        if (category === 'Combustível')
            return `Gasolina — ${merchant}`;
        if (category === 'Saúde')
            return `Farmácia/saúde — ${merchant}`;
        if (category === 'Alimentação')
            return `Alimentação — ${merchant}`;
        if (category === 'Mercado')
            return `Mercado — ${merchant}`;
        if (category === 'Transferências')
            return `Transferência — ${merchant}`;
        if (category === 'Assinaturas')
            return `Assinatura — ${merchant}`;
        if (category === 'Fatura cartão')
            return `Fatura cartão — ${merchant}`;
    ...
    isConsumptionDebit(e, monthsWithCard) {
        ...
        if ((0, category_guess_1.isInvestmentMovement)(e.description))
            return false;
        if ((0, category_guess_1.inferSpendingCategory)(e.description) === 'Investimentos')
            return false;
        const merchant = this.merchantLabel(e.description);
        if ((0, category_guess_1.inferSpendingCategory)(merchant) === 'Investimentos')
            return false;
        return true;
    }
    composeMovementSummary(input) {
        const bullets = [];
        const m = input.refBreakdown;
        if (!m)
            return bullets;
        ...
        if (m.investedApplied > 0 || m.investedRedeemed > 0) {
            bullets.push(`Em ${refLabel}: aplicou ${this.brl(m.investedApplied)} e resgatou ${this.brl(m.investedRedeemed)} no RDB.`);
        }
        if (input.currentlyInvested > 0) {
            bullets.push(`${this.brl(input.currentlyInvested)} ainda aplicados no RDB.`);
        }
        if (m.net < 0) {
            bullets.push(`Fluxo bruto negativo (${this.brl(Math.abs(m.net))}) — inclui investimentos; consumo está na análise acima.`);
        }
        return bullets.slice(0, 3);
    }
    buildDirectActions(input) {
        const candidates = [];
    ...
    buildDeficitReason(input) {
        ...
        if (input.topCategories.length > 0) {
            ...
            parts.push(`Principais destinos: ${top3
        ...
        if (transfers > input.overspendAmount * 0.3) {
            parts.push(`Pix/transferências enviadas somaram ${this.brl(transfers)} — parte relevante do excesso de saída.`);
        }
        if (cardBill > 0) {
            parts.push(`Pagamento de fatura do cartão: ${this.brl(cardBill)}.`);
        }
        if (invested > 0) {
            parts.push(`Houve também ${this.brl(invested)} aplicados em RDB (investimento, não consumo).`);
        }
        if (input.largestExpense && input.largestExpense.amount >= input.overspendAmount * 0.4) {
            parts.push(`O maior lançamento isolado foi ${this.brl(input.largestExpense.amount)} (${input.largestExpense.label}).`);
        }
        return parts.join(' ');
    }
    groupRecurringExpenses(entries, contextRules) {
        const map = new Map();
        for (const e of entries) {
            const label = this.merchantLabel(e.description);
            const rule = (0, finance_context_matcher_1.findMatchingRule)(contextRules, e.description, label);
            const fromDesc = (0, category_guess_1.inferSpendingCategory)(e.description);
            const inferred = fromDesc !== 'Outros' ? fromDesc : (0, category_guess_1.inferSpendingCategory)(label);
            const category = rule?.category ?? inferred;
        ...
        return [...map.values()].map((g) => ({
            ...
            averageAmount: this.round(g.amounts.reduce((s, a) => s + a, 0) / Math.max(g.amounts.length, 1)),
        }));
    }
    toRecurringExpense(id, g) {
    ...
    summarizeExpenses(entries, month) {
        const map = new Map();
        for (const e of entries) {
            if (e.month !== month || e.direction !== 'DEBIT')
                continue;
            if (e.type === 'INVESTMENT_APPLY')
                continue;
    ...
    buildHealthObservations(analysis) {
        if (!analysis.hasData) {
            return [
                {
                    id: 'bank-no-data',
                    ...
                    message: 'Importe seus extratos bancários para insights baseados nos lançamentos reais.',
                ...
                },
                {
                    ...
                    message: 'Esta seção usa exclusivamente movimentações reais do banco — não despesas cadastradas manualmente.',
                ...
                },
                {
                    ...
                    message: 'Depois de importar, use "Ensinar" em cada gasto para a IA entender Pix e transferências.',
        ...
        if (spend?.spentMoreThanEarned) {
            add('CRITICAL', 'Gastos acima das entradas no extrato', spend.deficitReason, spend.actions[0]);
        }
        if (spend?.topCategories[0]) {
            const top = spend.topCategories[0];
            if (top.sharePercent >= 25) {
                add('ATTENTION', `${top.category} concentra seus gastos`, `${this.brl(top.total)} (${top.sharePercent.toFixed(0)}%) do consumo em ${refLabel} — ${top.count} lançamentos no extrato.`, spend.actions.find((a) => a.toLowerCase().includes(top.category.toLowerCase())) ?? `Revise os lançamentos em ${top.category} no extrato.`);
            }
        }
        if (spend) {
            const topRec = [...spend.recurringExpenses]
                ...
                .sort((a, b) => b.totalInPeriod - a.totalInPeriod)[0];
            if (topRec) {
                add('ATTENTION', `Recorrente no extrato: ${topRec.label}`, `${topRec.occurrences}x identificado(s), média ${this.brl(topRec.averageAmount)} — impacto anual ~${this.brl(topRec.annualizedEstimate)}.`, `Confira se ${topRec.label} ainda faz sentido ou se dá para negociar.`);
            }
        }
        if (spend?.largestExpense && spend.largestExpense.amount >= 100) {
            const l = spend.largestExpense;
            add('INFO', 'Maior gasto do mês no extrato', `${l.label}: ${this.brl(l.amount)} em ${this.brDate(l.date)} [${l.category}].`, spend.actions[0]);
        }
        if (ref && ref.totalIn > 0 && ref.net > 0) {
            const savedPct = Math.round((ref.net / ref.totalIn) * 100);
            if (savedPct >= 8) {
                add('POSITIVE', 'Sobra no extrato este mês', `Entradas ${this.brl(ref.totalIn)}, saídas ${this.brl(ref.totalOut)} — sobraram ${this.brl(ref.net)} (${savedPct}% das entradas).`);
            }
        }
        if (analysis.currentlyInvested > 0) {
            add('POSITIVE', 'Patrimônio aplicado (extrato)', `${this.brl(analysis.currentlyInvested)} ainda investidos em RDB, rastreados pelos lançamentos importados.`, analysis.recommendations[0]);
        }
        if (ref && ref.investedApplied > 0) {
            add(ref.investedRedeemed > ref.investedApplied ? 'ATTENTION' : 'POSITIVE', `Movimentação de investimento em ${refLabel}`, `Aplicou ${this.brl(ref.investedApplied)}${ref.investedRedeemed > 0 ? ` e resgatou ${this.brl(ref.investedRedeemed)}` : ''} — dados do extrato.`, 'Evite resgates parciais antes do prazo para não perder rendimento.');
        }
    ...
    buildHealthSummary(scoreValue, delta, trend, analysis) {
        let summary;
        if (scoreValue >= 75) {
            ...
            insights: [
                {
                    ...
                    tone: 'INFO',
                    title: 'Nenhum extrato importado',
                    message: 'Importe seus extratos bancários (CSV/OFX) para a IA analisar entradas, saídas, investimentos e resgates entre meses.',
...
exports.BankStatementAnalysisService = BankStatementAnalysisService;
exports.BankStatementAnalysisService = BankStatementAnalysisService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], BankStatementAnalysisService);
//# sourceMappingURL=bank-statement-analysis.service.js.map