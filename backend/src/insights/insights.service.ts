...
export class InsightsService {
  ...
  /** Generates plain-language observations — only what matters for this user. */
  private composeHealthObservations(input: {
    recent: MonthlySnapshot[];
    ...
    installments: { totalPending: number; activeGroups: number };
    investments: InvestmentAnalysis;
  }): { summary: string; observations: HealthObservation[] } {
    ...
    if (recent.length === 0) {
      return {
        summary:
          'Ainda não temos dados suficientes. Cadastre receitas e despesas para a análise começar.',
        observations: [
          {
            id: 'health-no-data',
            tone: 'INFO',
            ...
            message:
              'Com receitas e despesas lançadas, conseguimos explicar sua situação em linguagem simples e apontar o que merece atenção.',
    ...
    if (!ind.hasRegisteredContributions) {
      observations.push({
        ...
        message:
          'Não encontramos aportes individuais cadastrados. A IA só analisa o que você registra na tela de Investimentos.',
    ...
    const toneOrder: Record<HealthObservation['tone'], number> = {
      CRITICAL: 0,
      ATTENTION: 1,
      INFO: 2,
      POSITIVE: 3,
  ...
  private computeHabits(
    ...
    if (investments.individual.consecutiveMonths >= 3) {
      ...
      attentions.push(
        'Nenhum aporte individual registrado. Use a tela Investimentos → Individual.',
      );
